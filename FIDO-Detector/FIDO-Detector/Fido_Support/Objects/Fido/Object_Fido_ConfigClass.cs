...
namespace FIDO_Detector.Fido_Support.Objects.Fido
{
  public class Object_Fido_ConfigClass
  {
    ...
    public List<ParseConfigs> FormatParsePan(DataTable dbReturn)
    {

      var reFormatReturn = new List<ParseConfigs>();

      for (var i = 0; i < dbReturn.Rows.Count; i++)
      {
        try
        {
          var reformat = new ParseConfigs
          {
            DetectorType = Convert.ToString(dbReturn.Rows[i].ItemArray[1]),
            Detector = Convert.ToString(dbReturn.Rows[i].ItemArray[2]),
            Vendor = Convert.ToString(dbReturn.Rows[i].ItemArray[3]),
            Server = Convert.ToString(dbReturn.Rows[i].ItemArray[4]),
            Folder = Convert.ToString(dbReturn.Rows[i].ItemArray[5]),
            FolderTest = Convert.ToString(dbReturn.Rows[i].ItemArray[6]),
            File = Convert.ToString(dbReturn.Rows[i].ItemArray[7]),
            EmailFrom = Convert.ToString(dbReturn.Rows[i].ItemArray[8]),
            Lastevent = Convert.ToString(dbReturn.Rows[i].ItemArray[9]),
            UserID = Convert.ToString(dbReturn.Rows[i].ItemArray[10]),
            Pwd = Convert.ToString(dbReturn.Rows[i].ItemArray[11]),
            Acek = Convert.ToString(dbReturn.Rows[i].ItemArray[12]),
            DB = Convert.ToString(dbReturn.Rows[i].ItemArray[13]),
            ConnString = Convert.ToString(dbReturn.Rows[i].ItemArray[14]),
            Query = Convert.ToString(dbReturn.Rows[i].ItemArray[15]),
            Query2 = Convert.ToString(dbReturn.Rows[i].ItemArray[16]),
            Query3 = Convert.ToString(dbReturn.Rows[i].ItemArray[17]),
            APIKey = Convert.ToString(dbReturn.Rows[i].ItemArray[18])
          };
          reFormatReturn.Add(reformat);
        }
        catch (Exception e)
        {
      ...
      return reFormatReturn;

    }

    public ParseConfigs FormatParse(DataTable dbReturn)
    {
      try
      {
        var reformat = new ParseConfigs
