...
namespace Fido_Main.Director.Scoring
{
  internal static class Matrix
  {
    public static FidoReturnValues RunMatrix(FidoReturnValues lFidoReturnValues)
    {
      ...
      var isRunAssett = new SqLiteDB().ExecuteBool(@"select assetscore from configs_director");
      if (isRunAssett)
      {
        #region AssetValue
        Console.WriteLine(@"Starting assest evaluation.");
        //asset evaluation
        var isPaired = new SqLiteDB().ExecuteBool(@"select paired from configs_posture_asset");
        ...
        //AV evaluation
        lFidoReturnValues = Matrix_Scoring.GetCBScore(lFidoReturnValues);
