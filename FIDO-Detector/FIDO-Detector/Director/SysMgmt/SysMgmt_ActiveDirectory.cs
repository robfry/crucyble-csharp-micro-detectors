  ...
  static class SysMgmt_ActiveDirectory
  {
    public static UserReturnValues Getuserinfo(string sUserId)
    {
      try
      {
        ...
        var domainPath = new SqLiteDB().ExecuteScalar(@"select basedn from configs_ldap");
        var user = new SqLiteDB().ExecuteScalar(@"select userid from configs_ldap");
        user = Base64.Decode(user);
        var pwd = new SqLiteDB().ExecuteScalar(@"select pwd from configs_ldap");
