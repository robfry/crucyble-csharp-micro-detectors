  ...
  class Notification_CarbonBlack_Helper
  {
    ...
    private static Dictionary<string, string> CarbonBlackBadGuyReplacements(FidoReturnValues lFidoReturnValues, Dictionary<string, string> replacements)
    {
      try
      {
        ...
        if (integrationConfigs.rows[0].value.threatstack.virustotal) replacements = CarbonBlackVTReplacements(lFidoReturnValues, replacements);
          //if (new SqLiteDB().ExecuteBool(@"select virustotal from configs_director")) replacements = CarbonBlackVTReplacements(lFidoReturnValues, replacements);

        if (integrationConfigs.rows[0].value.threatstack.threatgrid) replacements = CarbonBlackGeoReplacements(lFidoReturnValues, replacements);
          //if (new SqLiteDB().ExecuteBool(@"select threatgrid from configs_director")) replacements = CarbonBlackGeoReplacements(lFidoReturnValues, replacements);

        if (integrationConfigs.rows[0].value.threatstack.threatgrid) replacements = CarbonBlackThreatGRIDReplacements(lFidoReturnValues, replacements);
          //if (new SqLiteDB().ExecuteBool(@"select threatgrid from configs_director")) replacements = CarbonBlackThreatGRIDReplacements(lFidoReturnValues, replacements);
    ...
    {
      if (lFidoReturnValues.CB.Alert.ThreatGRID != null && lFidoReturnValues.CB.Alert.ThreatGRID.ThreatScore > 0)
      {
      ...
      if (lFidoReturnValues.CB.Alert.ThreatGRID != null && lFidoReturnValues.CB.Alert.ThreatGRID.ThreatSeverity > 0)
      {
        if ((lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo != null) && (lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo.Count > 0))
        {
      ...
      if (lFidoReturnValues.CB.Alert.ThreatGRID != null && lFidoReturnValues.CB.Alert.ThreatGRID.ThreatConfidence > 0)
      {
        if ((lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo != null) && (lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo.Count > 0))
        {
      ...
      if (lFidoReturnValues.CB.Alert.ThreatGRID != null && lFidoReturnValues.CB.Alert.ThreatGRID.ThreatIndicators > 0)
      {
        if ((lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo != null) && (lFidoReturnValues.CB.Alert.ThreatGRID.IPThreatInfo.Count > 0))
        {
